...
// ══════════════════════════════════════════════
//  BRANDING — edit these 4 lines to rebrand
// ══════════════════════════════════════════════
...
// API endpoint — goes through Netlify Function (API key stays secret on server)
...
const EMERGENCY_WORDS = [
  ...
  "unconscious","not breathing","severe bleeding","suicide","overdose",
  "seizure","choking","poisoning","anaphylaxis","unresponsive","dying",
];
...
${name ? `Patient name: ${name}. Use their name naturally to personalize responses.` : ""}

Specialties: General Medicine, Cardiology, Neurology, Orthopedics, Dermatology, Gastroenterology, Psychiatry, Pediatrics, Gynecology, Urology, ENT, Ophthalmology, Dentistry, Endocrinology, Oncology, Pulmonology, Rheumatology, Nephrology, Infectious Diseases, Emergency Medicine, Sports Medicine, Nutrition, and more.

Approach:
1. Carefully analyze the patient's described symptoms
2. Ask 1-2 smart follow-up questions when needed (age, duration, severity, medical history)
3. Provide a professional structured analysis:
   🔍 **Possible Conditions** — ranked by likelihood with brief explanation
...
4. Be empathetic, clear, and thorough — like a top doctor talking to a patient
5. For emergencies: IMMEDIATELY urge calling emergency services (911 / 999 / 112)

Always close with: "⚠️ This analysis is informational guidance only. Please consult a licensed medical professional for official diagnosis and treatment."`;

const mkId = () => Math.random().toString(36).slice(2);
const mkConv = (specialty, name) => ({
  id: mkId(), title: "New Consultation",
  messages: [], specialty: specialty || null, createdAt: new Date(),
});
  ...
  <div style={{ lineHeight: 1.8, fontSize: "0.87rem" }}>
    {text.split("\n").map((line, i) => {
      ...
      const html = line
        ...
        .replace(/\*(.*?)\*/g, "<em>$1</em>");
      if (/^#{1,3}\s/.test(line))
        return <p key={i} style={{ color:"#d8b4fe", fontWeight:700, margin:"10px 0 4px", fontSize:"0.92rem" }} dangerouslySetInnerHTML={{ __html: html.replace(/^#+\s/,"") }} />;
      if (/^[-•]\s/.test(line))
        return <div key={i} style={{ display:"flex", gap:8, marginBottom:3 }}><span style={{ color:"#a855f7", flexShrink:0 }}>•</span><span dangerouslySetInnerHTML={{ __html: html.replace(/^[-•]\s/,"") }} /></div>;
      if (/^\d+\.\s/.test(line))
        return <div key={i} style={{ display:"flex", gap:8, marginBottom:3 }}><span style={{ color:"#a855f7", flexShrink:0, minWidth:14 }}>{(line.match(/^\d+/)||[""])[0]}.</span><span dangerouslySetInnerHTML={{ __html: html.replace(/^\d+\.\s/,"") }} /></div>;
...
const Dots = () => (
  <div style={{ display:"flex", gap:5, padding:"2px 0" }}>
    {[0,1,2].map(i => (
      <span key={i} style={{ width:7, height:7, borderRadius:"50%", background:"#a855f7", display:"block",
...
export default function App() {
  ...
  const send = async (textOverride) => {
    ...
    try {
      ...
      updateConv(currentId, c => ({
        ...c, messages: [...c.messages, { role: "assistant", content: reply, id: mkId() }],
      }));
    } catch (err) {
  ...
  const exportChat = () => {
    ...
    const lines = msgs.map(m =>
      `[${m.role === "user" ? (userName || "Patient") : BRAND_NAME}]\n${m.content}`
    ).join("\n\n" + "─".repeat(40) + "\n\n");
    const txt = [
      `${BRAND_NAME} — Medical Consultation Report`,
      "═".repeat(45),
    ...
    a.download = `${BRAND_NAME}-report-${Date.now()}.txt`;
    a.click();
  };
  ...
  const css = `
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    ...
    body{background:#080212;font-family:'DM Sans',sans-serif;-webkit-font-smoothing:antialiased;}
    ...
    @keyframes emergFlash{0%,100%{background:rgba(239,68,68,.1)}50%{background:rgba(239,68,68,.22)}}
    ...
    <>
      ...
      <div style={{ minHeight:"100vh", background:"radial-gradient(ellipse at 30% 10%,#1e0845 0%,#080212 55%,#0e0325 100%)", display:"flex", alignItems:"center", justifyContent:"center", padding:24, overflow:"hidden", position:"relative" }}>
        ...
        <div style={{ width:"100%", maxWidth:420, textAlign:"center", animation:"fadeUp .6s ease" }}>
          <div style={{ width:90, height:90, borderRadius:28, background:"linear-gradient(135deg,#4c1d95,#7c3aed,#a855f7)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"2.6rem", margin:"0 auto 24px", animation:"glow 3s ease infinite, float 5s ease-in-out infinite" }}>
            {BRAND_ICON}
          </div>
          <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:"2.4rem", background:"linear-gradient(135deg,#f3e8ff,#c084fc,#7c3aed)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", marginBottom:6 }}>
            {BRAND_NAME}
          </h1>
          <p style={{ color:"rgba(196,181,253,.55)", fontSize:".9rem", marginBottom:36, letterSpacing:".04em" }}>{BRAND_TAGLINE}</p>
          ...
          <div style={{ background:"rgba(255,255,255,.03)", border:"1px solid rgba(168,85,247,.14)", borderRadius:20, padding:"30px 26px", backdropFilter:"blur(20px)" }}>
            ...
            />
            <p style={{ color:"rgba(196,181,253,.3)", fontSize:".7rem", margin:"10px 0 22px" }}>
              Stays private. Helps personalize your health consultation.
            </p>
    ...
    <>
      ...
      <div style={{ height:"100vh", display:"flex", background:"radial-gradient(ellipse at 20% 0%,#150430 0%,#080212 60%)", color:"#f3e8ff", overflow:"hidden" }}>
        ...
        <aside style={{ width:sidebarOpen?248:0, minWidth:sidebarOpen?248:0, overflow:"hidden", transition:"all .25s ease", background:"rgba(0,0,0,.32)", borderRight:"1px solid rgba(168,85,247,.09)", display:"flex", flexDirection:"column", flexShrink:0 }}>
          <div style={{ padding:"18px 14px 14px", borderBottom:"1px solid rgba(168,85,247,.08)" }}>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14 }}>
              <div style={{ width:34, height:34, borderRadius:10, background:"linear-gradient(135deg,#5b21b6,#7c3aed)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"1.1rem", flexShrink:0 }}>
                {BRAND_ICON}
              </div>
              <div>
          ...
          <div style={{ padding:"12px 14px 6px", borderBottom:"1px solid rgba(168,85,247,.06)" }}>
            ...
            <div style={{ maxHeight:190, overflowY:"auto" }}>
              {SPECIALTIES.map(s => (
                <div key={s.name}
                  className={`spec-item${specialty === s.name ? " sel" : ""}`}
          ...
          <div style={{ flex:1, overflowY:"auto", padding:"10px 14px" }}>
            ...
            {convos.length === 0 && (
              <p style={{ fontSize:".75rem", color:"rgba(196,181,253,.22)", padding:"4px 0" }}>No history yet</p>
            )}
            {convos.map(c => (
              <button key={c.id} className={`sbtn${c.id === currentId ? " active" : ""}`}
                onClick={() => { setCurrentId(c.id); setEmergency(false); }}>
          ...
          <div style={{ padding:"10px 14px", borderTop:"1px solid rgba(168,85,247,.07)", fontSize:".64rem", color:"rgba(196,181,253,.22)", lineHeight:1.55 }}>
            ⚠ Informational use only.<br />Not a substitute for professional medical care.
          </div>
        </aside>
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", minWidth:0 }}>
          ...
          {/* Header */}
          <header style={{ padding:"13px 18px", borderBottom:"1px solid rgba(168,85,237,.09)", background:"rgba(8,2,18,.8)", backdropFilter:"blur(20px)", display:"flex", alignItems:"center", gap:10, flexShrink:0 }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              {current?.specialty && (
                <div style={{ fontSize:".65rem", color:"#9f7aea" }}>
                  {SPECIALTIES.find(s => s.name === current.specialty)?.icon} {current.specialty}
                </div>
              )}
            </div>
            <div style={{ display:"flex", gap:4, alignItems:"center" }}>
              {msgs.length > 0 && (
                <>
                  <button className="ibtn" title="Export report as .txt" onClick={exportChat}>📄</button>
            ...
            <div style={{ background:"rgba(239,68,68,.1)", borderBottom:"1px solid rgba(239,68,68,.28)", padding:"10px 18px", display:"flex", alignItems:"center", justifyContent:"space-between", animation:"emergFlash 2s ease infinite", flexShrink:0 }}>
              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                ...
                <div>
                  ...
                  <div style={{ color:"rgba(252,165,165,.65)", fontSize:".73rem" }}>
                    Call emergency services → <strong>911</strong> (US) · <strong>999</strong> (UK) · <strong>112</strong> (EU/India)
                  </div>
                </div>
              </div>
          ...
          {/* API Error Banner */}
          {apiError && (
            <div style={{ background:"rgba(234,179,8,.08)", borderBottom:"1px solid rgba(234,179,8,.25)", padding:"8px 18px", fontSize:".75rem", color:"#fde047", flexShrink:0 }}>
              ⚠ {apiError.includes("API key") ? "API key not set — add ANTHROPIC_API_KEY in Netlify → Site Settings → Environment Variables." : apiError}
            </div>
          )}
          ...
          <div style={{ flex:1, overflowY:"auto", padding:"22px 18px", display:"flex", flexDirection:"column", gap:16 }}>
            ...
            {msgs.length === 0 && (
              <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", textAlign:"center", padding:"32px 16px", animation:"fadeUp .5s ease" }}>
                <div style={{ width:80, height:80, borderRadius:24, background:"linear-gradient(135deg,#4c1d95,#7c3aed)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"2.4rem", marginBottom:20, boxShadow:"0 0 40px rgba(124,58,237,.45)", animation:"float 5s ease-in-out infinite" }}>
                  {BRAND_ICON}
                </div>
                ...
                <p style={{ color:"rgba(196,181,253,.5)", fontSize:".87rem", maxWidth:360, lineHeight:1.7, marginBottom:26 }}>
                  Describe your symptoms in detail. I'll analyze them with expert-level knowledge across{" "}
                  <strong style={{ color:"#a855f7" }}>all 18 medical specialties</strong>.
                </p>
                ...
                <div style={{ display:"flex", flexWrap:"wrap", gap:7, justifyContent:"center", maxWidth:480, marginBottom:26 }}>
                  {SPECIALTIES.slice(0, 10).map(s => (
                    <span key={s.name} className={`chip${specialty === s.name ? " sel" : ""}`}
                      onClick={() => setSpecialty(prev => prev === s.name ? null : s.name)}>
                      {s.icon} {s.name}
                    </span>
                  ))}
                </div>
            ...
            {msgs.map(msg => (
              <div key={msg.id} className="msg-wrap msg" style={{ display:"flex", flexDirection:msg.role === "user" ? "row-reverse" : "row", gap:10, alignItems:"flex-start" }}>
                ...
                <div style={{ maxWidth:"76%", position:"relative" }}>
                  ...
                  <button className="ibtn copy-btn" style={{ position:"absolute", top:6, [msg.role === "user" ? "left" : "right"]:-32, fontSize:".75rem", background:"rgba(0,0,0,.4)", border:"1px solid rgba(168,85,247,.18)", padding:"4px 7px" }}
                    onClick={() => copyMsg(msg.content, msg.id)} title="Copy">
                    {copiedId === msg.id ? "✓" : "⎘"}
                  </button>
                </div>
              </div>
            ...
            {loading && (
              <div className="msg" style={{ display:"flex", gap:10, alignItems:"flex-start" }}>
                <div style={{ width:34, height:34, borderRadius:11, background:"linear-gradient(135deg,#5b21b6,#7c3aed)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, boxShadow:"0 0 14px rgba(124,58,237,.3)" }}>
                  {BRAND_ICON}
                </div>
          ...
          {/* Disclaimer */}
          {msgs.length > 0 && (
            <div style={{ textAlign:"center", padding:"5px 14px", fontSize:".63rem", color:"rgba(196,181,253,.2)", borderTop:"1px solid rgba(255,255,255,.03)", flexShrink:0 }}>
              ⚠ {BRAND_NAME} provides informational analysis only — not a substitute for licensed medical care
            </div>
          )}

          {/* Input bar */}
          <div style={{ padding:"12px 18px 16px", background:"rgba(8,2,18,.85)", backdropFilter:"blur(20px)", borderTop:"1px solid rgba(168,85,247,.09)", flexShrink:0 }}>
            {specialty && (
              <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:8 }}>
                ...
                <button style={{ background:"none", border:"none", color:"rgba(196,181,253,.35)", cursor:"pointer", fontSize:".7rem" }}
                  onClick={() => setSpecialty(null)}>✕ clear</button>
              </div>
            )}
            ...
            <div style={{ display:"flex", gap:8, alignItems:"flex-end", background:"rgba(255,255,255,.04)", border:"1px solid rgba(168,85,247,.2)", borderRadius:16, padding:"11px 13px", boxShadow:"0 0 24px rgba(124,58,237,.08)" }}>
              <button className="ibtn" onClick={toggleVoice} title={isListening ? "Stop" : "Voice input"}
                style={{ color:isListening ? "#f87171" : "rgba(196,181,253,.5)", animation:isListening ? "pulse 1s infinite" : "none", flexShrink:0 }}>
                🎙
              </button>
              <textarea
                ref={inputRef}
            ...
            <div style={{ display:"flex", justifyContent:"space-between", marginTop:6 }}>
              ...
              <span style={{ fontSize:".62rem", color:"rgba(196,181,253,.18)" }}>Powered by {BRAND_NAME}</span>
            </div>
          </div>
        </div>
      </div>
